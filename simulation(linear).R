...
iter.num = 14
simulation.list = foreach(iter = 1:iter.num) %dopar% {
  # Data Generation
  ...
  # Basis Evaluation
  splat.collection.total.2D = MakeSplatCollection2D(splat.collection = splat.collection.record, data = sim.image, curve.fitting = FALSE)
  ...
  foo = list(data = sim.image, splat.collection.init = splat.collection, splat.collection.total = splat.collection.record, splat.collection.total.2D = splat.collection.total.2D, eval.mat.full = eval.mat.obs, result.tps = result.tps)
